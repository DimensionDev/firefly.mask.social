...
import { FireflyIconPaletteContext } from './FireflyIconPaletteContext.js';
...
export function __createIcon(name, variants, intrinsicSize = [24, 24]) {
    function Icon(/** @type {import('./internal').GeneratedIconProps} */ props, ref) {
        ...
        const iconProps = {
            'aria-hidden': true,
            ...rest,
            className: `inline-block bg-no-repeat bg-center flex-shrink-0 ${rest.className || ''}`,
            ref,
            'data-icon': name,
            style: iconStyle,
        ...
        if (supportColor && jsx) {
            ...
            return React.createElement('span', iconProps, jsx);
        }
        iconProps.component = 'span';
...
function useDefaultPalette() {
    const palette = React.useContext(FireflyIconPaletteContext);
    return palette;
}
