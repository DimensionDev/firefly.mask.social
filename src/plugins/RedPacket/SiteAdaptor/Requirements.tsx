...
import { 
    type ClaimStrategyStatus,
type PostReactionKind,
PlatformType,
} from '@/providers/types/RedPacket.js';

import { RedPacketTrans, useRedPacketTrans } from '@/plugins/RedPacket/locales/i18n_generated.js';
import { usePlatformType } from '@/plugins/RedPacket/SiteAdaptor/hooks/usePlatformType.js';
...
const IconMap: Record<PostReactionKind, GeneratedIcon> = {
    like: Icons.Heart,
    repost: Icons.Repost,
    quote: Icons.Repost,
...
function resolveProfileUrl(platform: PlatformType, handle: string) {
    switch (platform) {
            ...
            {payload.map(({ handle, profileId, platform }) => (
                <Link
                    ...
                    href={resolveProfileUrl(
                        platform,
                        platform === PlatformType.farcaster ? profileId : handle,
                    )}
                    target="_blank"
...
export const Requirements = forwardRef<HTMLDivElement, Props>(function Requirements(
    ...
    const platform = usePlatformType() as PlatformType;
