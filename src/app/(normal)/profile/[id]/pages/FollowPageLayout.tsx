...
import { FollowCategory, type SocialSource, Source } from '@/constants/enum.js';
...
export function FollowPageLayout({ id, source, children }: PropsWithChildren<{ id: string; source: SocialSource }>) {
        ...
        <>
            ...
            <nav className="border-b border-line bg-primaryBottom px-4">
                <ul className="scrollable-tab -mb-px flex space-x-4" aria-label="Tabs">
                    {tabs.map((tab) => {
                        const path = `/profile/${id}/${tab.category}`;
                        return (
                            <li
                                key={tab.category}
