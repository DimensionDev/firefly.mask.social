...
export function NotificationFilter({ source, className, types, onTypesChange: setTypes, ...props }: Props) {
    ...
    const tabs = useMemo(() => {
        const getClassNames = (active: boolean) => {
            return classNames(
                'flex h-6 cursor-pointer items-center justify-center gap-x-1 rounded-md bg-farcasterPrimary px-1.5 text-xs leading-6 whitespace-nowrap',
                active
    ...
    return (
        <div className={classNames('flex gap-x-2 w-full overflow-x-auto no-scrollbar', className)} {...props}>
            {tabs}
        </div>
    );
}
