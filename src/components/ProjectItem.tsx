...
export const ProjectItem = memo<{ project: Project }>(function ProjectItem({
    project: { project_name, logo, token_symbol, eval: evalNumber, rank, tags, one_liner, project_id },
}) {
    const [{ loading }, handleClick] = useAsyncFn(async () => {
        const detail = await FireflyEndpointProvider.getProjectDetail(project_id);

        openWindow(detail.rootdataurl, '_blank');
    }, [project_id]);
    return (
        <div
            className={classNames('flex cursor-pointer gap-x-2 border-b border-line px-3 py-2 hover:bg-bg', {
